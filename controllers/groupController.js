...
async function createAdminGroup(req, res) {
  try {
    ...
    if (err.code === 11000) {
      return res
        .status(400)
        .json({ message: "Admin group name already exists." });
    }
...
async function createUnitManagerGroup(req, res) {
  try {
    ...
    res.status(201).json({
      message: "Unit manager group created successfully.",
      group,
    });
  } catch (err) {
    ...
    if (err.code === 11000) {
      return res.status(400).json({
        message: "Unit manager group name already exists.",
      });
    }

    res.status(500).json({
      message: "Failed to create unit manager group.",
    });
  }
}
...
async function addAdminToGroup(req, res) {
  try {
    ...
    if (user.role !== "ADMIN") {
      return res.status(400).json({
        message: "Only ADMIN users can be added to AdminGroup.",
      });
    }
    ...
    res
      .status(200)
      .json({ message: "Admin added to group successfully.", group });
  } catch (err) {
...
async function addUnitManagerToGroup(req, res) {
  try {
    ...
    if (!group) {
      return res
        .status(404)
        .json({ message: "Unit manager group not found." });
    }
    ...
    if (user.role !== "UNIT_MANAGER") {
      return res.status(400).json({
        message: "Only UNIT_MANAGER users can be added to UnitManagerGroup.",
      });
    }
    ...
    res.status(500).json({
      message: "Failed to add unit manager to group.",
    });
  }
}
