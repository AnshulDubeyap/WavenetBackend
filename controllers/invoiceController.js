  ...
  try {
    ...
    if (!invoiceNumber || !date || amount == null) {
      return res.status(400).json({
        message: "invoiceNumber, date and amount are required.",
      });
    }
...
async function deleteInvoices(req, res) {
  try {
    ...
    if (!Array.isArray(invoiceNumbers) || invoiceNumbers.length === 0) {
      return res.status(400).json({
        message: "Array of invoiceNumbers is required.",
      });
    }
    ...
    res.status(200).json({
      message:
        "Invoices deleted successfully (only those you had permission for).",
